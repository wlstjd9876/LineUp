			...
			success:function(data){
				...
				if(count<0 || list == null){
					...
					$(list).each(function(index,app){
						   ...
						   if(app.app_status == 0)
							   output += '		<div class="cell"><button type="button" class="btn btn-success btn-sm" onclick="location.href=\''+contextPath+'/teamapp/lineup.do?app_num='+app.app_num+'&id='+app.app_id+'&status=0\'">LineUp!</button></div>';
						   else
							   output += '		<div class="cell"><button type="button" class="btn btn-danger btn-sm" onclick="location.href=\''+contextPath+'/teamapp/lineup.do?app_num='+app.app_num+'&id='+app.app_id+'&status=1\'">Fire!</button></div>';
						   output += '		<div class="cell"><button type="button" class="btn btn-danger btn-sm" onclick="location.href=\''+contextPath+'/teamapp/delete.do?app_num='+app.app_num+'&id='+app.app_id+'\'">Cancel</button></div>';
						   output += '	</div>';
					});
					output += '</div>';
