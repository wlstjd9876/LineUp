$(document).ready(function(){
	var Phoneduplicated = 1;
	var Emailduplicated = 1;
	var Nickduplicated = 1;
	var pw_check = 0;
	...
	$('#phone').keyup(function(){	
		...
		$.ajax({
			url:'solo_modifycheckDuplicatedElements.do',
			type:'post',
	...
	$('#nick').keyup(function(){
		...
		$.ajax({
			...
			timeout:30000,
			success:function(data){
				$('#loading_nick').hide();//로딩 이미지 감추기
				if(data.result == 'NotFound'){
					$('#message_nick').css('color','blue').text('사용 가능한 닉네임이네요!!');
					Nickduplicated = 1;
				}else if(data.result == 'Duplicated'){
					$('#message_nick').css('color','red').text('이미 있는 닉네임입니다!!');
					Nickduplicated = 0;
				}else{
